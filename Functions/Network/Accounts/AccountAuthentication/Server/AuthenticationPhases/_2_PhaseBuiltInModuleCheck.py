...
def _2_PhaseBuiltInModuleCheck(s: socket.socket, logs: Logs, Authentication: 'Auth') -> bool:
    ...
    message: str = Authentication._getMessage(s)
    if message is None:
        s.close()
        ...
        return False
    try:
        message: dict[str, str] = literal_eval(message.replace(" ", ''))  # making it type of dict
    except ValueError:
        s.close()
        logs.sendLog("[Authentication] Couldn't pass 2nd authentication phase."
                     f"{s.getpeername()[0]}:{s.getpeername()[1]}", -1)
    ...
    for i in modules:  # comparing versions from ours to client
        if i == 'UI':
            continue
        if modules.get(i) is not None and modules.get(i) != message.get(i):  # if version is not the same
