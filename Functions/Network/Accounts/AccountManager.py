...
class AccountManager(AccountDataTransfer, AccountDataHandler):
    ...
    def closeConnection(self):
        ...
        if self.getIsServer():
            self._serverStopped()
            for i in self.participants.copy():
    ...
    def _disconnectAccount(self, account: Account):
        ...
        if self.getIsServer():
            ...
            try:
                account.socket.socket.shutdown(socket.SHUT_RDWR)
            except OSError:
                pass
    ...
    def _disconnectSelfAccount(self):
        ...
        for i in temp.extraConnections.values():
            for _ in i:
                try:
                    _.socket.shutdown(socket.SHUT_RDWR)
                except OSError:
                    pass
        temp.extraConnections.clear()
        try:
            # TODO: Make timer
            if self.owner is temp:  # self.getIsServer() is not available
                temp.socket.socket.close()
            else:
                temp.socket.socket.shutdown(socket.SHUT_RDWR)
        except OSError:
            pass
